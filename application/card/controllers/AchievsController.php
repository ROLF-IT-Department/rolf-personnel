...
class Card_AchievsController extends Zend_Controller_Action
{
	...
	public function indexAction()
	{
		...
		$view->trainsGroupsMethodsActual = $trainsGroupsMethods->toArrayNamesWithoutDisabled($person->Platform);
	...
	private function _getUserRole(Rp_Db_Table_Row_Ach_Card $card, Rp_User $user)	// определяем какая роль у пользователя для данной карточки сотрудника
	{
		...
		// Подразумевалось, что можно было бы задавать всем HRовцам роль для добавления/блокировок карточек...
...
//		$user_employee_info = $employees->findByPersonId($userId)->current()->getPerson();
		...
		*/
		...
		// Подразумевалось, что можно было бы задавать всем HRовцам роль для добавления/блокировок карточек...
//		if (preg_match('/Департамент информационных технологий/', $user_employee_info->FullPath)
//		    OR preg_match('/Отдел по персоналу/', $user_employee_info->FullPath)
//		) {
